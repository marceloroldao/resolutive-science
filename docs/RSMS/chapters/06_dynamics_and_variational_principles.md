...
For declared bundles \(E_A\to M\), the configuration space is a specified subset
...
A boundary-value formulation specifies a domain \(\Omega\subseteq M\), boundary operator \(\mathcal B\), boundary data \(b\), and equations satisfying
