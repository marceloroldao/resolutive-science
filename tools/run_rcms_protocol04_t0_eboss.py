...
def main() -> None:
    # Likelihood-convention gates independent of cosmology.
    ...
    # Profile the common BAO scale and then optimize only the Lambda-CDM shape.
    result = minimize_scalar(
    ...
    if not np.isfinite(profile_span) or profile_span <= PROFILE_SPAN_MIN:
        raise SystemExit(
            f"shape identifiability gate failed: profile_span={profile_span:.8g}"
        )
