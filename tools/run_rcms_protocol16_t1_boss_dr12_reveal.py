#!/usr/bin/env python3
"""RCMS P16-T1 — preregistered BOSS DR12 held-out reveal."""
...
def model(rows, om, a, q):
    vals=[]; integ={}
    for z,_,kind in rows:
        e2=om*(1+z)**3 + 1-om + a*math.log1p(z)
        if e2<=0: raise ValueError
        e=math.sqrt(e2)
        if z not in integ: integ[z]=simpson_inv_e(z,om,a)
        if kind=="DM_over_rs": vals.append(q*RD_FID*integ[z])
        elif kind=="bao_Hz_rs": vals.append(C_KMS*e/(q*RD_FID))
        else: raise RuntimeError(kind)
    return np.asarray(vals)


def chi2(rows,data,ci,om,a,q):
    try: r=data-model(rows,om,a,q)
...
def profile_q(rows,data,ci,om,a):
    lq,val=golden(lambda x: chi2(rows,data,ci,om,a,math.exp(x)),math.log(Q_BOUNDS[0]),math.log(Q_BOUNDS[1]))
    return math.exp(lq),val


def profile_om(rows,data,ci,a):
    om,val=golden(lambda x: profile_q(rows,data,ci,x,a)[1],*OMEGA_BOUNDS)
    q,val=profile_q(rows,data,ci,om,a)
    return val,om,q


def main():
    rows,data,cov,ci=load()
    if cov.shape!=(6,6) or np.max(np.abs(cov-cov.T))>1e-10 or np.linalg.eigvalsh(cov).min()<=0:
    ...
    dchi=chi0-chi1
    daic=dchi-2.0
    dbic=dchi-math.log(6.0)

    if boundary:
        classification='OUT_OF_SAMPLE_INVALID'
    elif quantitative and directional:
        classification='OUT_OF_SAMPLE_QUANTITATIVE_HIT'
    elif a1<0:
        classification='OUT_OF_SAMPLE_SIGN_FAILURE'
    elif a1>=0 and zero_in and not quantitative:
        classification='OUT_OF_SAMPLE_NULL_COMPATIBLE'
    else:
        classification='OUT_OF_SAMPLE_DIRECTIONAL_ONLY'

    payload={
        'protocol':'P16-T1','status':'FINAL','target':'SDSS-III BOSS DR12 BAO-only consensus',
        'prediction_interval':list(PRED),'A_R_bounds':list(A_BOUNDS),'A_R_step':A_STEP,
    ...
    print('RCMS P16-T1 — BOSS DR12 blind reveal')
