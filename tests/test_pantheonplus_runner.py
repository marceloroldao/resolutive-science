"""Small-data unit tests for the Pantheon+ RCMS runner."""
...
class PantheonPlusRunnerTests(unittest.TestCase):
    def test_intercept_marginalization_is_shift_invariant(self) -> None:
        ...
        self.assertLess(abs(chi2), 1e-9)
        self.assertLess(abs(intercept - 23.4), 1e-9)

    def test_effective_model_recovers_lcdm_at_zero_amplitude(self) -> None:
    ...
    def test_global_magnitude_shift_is_absorbed_by_intercept(self) -> None:
        ...
        self.assertLess(abs(chi2_a - chi2_b), 1e-9)

    def test_nonpositive_expansion_is_rejected(self) -> None:
